...
module.exports = function Extract(moduleOptions) {
  ...
  this.nuxt.hook('export:routeCreated', async ({ route, path, errors }) => {
    ...
    fs.readFile(payloadPath, 'utf8', (err, data) => {
      ...
      let matches = data.matchAll(test)

      for (const match of matches) {
        const baseUrl = new URL(moduleOptions.baseUrl)
        const url = new URL(decodeURIComponent(JSON.parse('"' + match[0] + '"')))
      ...
      replacePayloadImageLinks(data, urls).then((payload) => {
        fs.writeFile(payloadPath, payload, 'utf8', function (err) {
           if (err) return console.log(err)
        })
      })
    })
